...
from flask_covid19.app_config.database import admin
...
from flask_covid19.app_web.web_dispachter_matrix_service import rki_service
...
from flask_covid19.data_rki.rki_model_flat import RkiFlat
from flask_covid19.data_rki.rki_model_import import RkiImport
from flask_covid19.data_rki.rki_service_test import RkiTestService
...
class RkiUrls:
    ...
    def url_rki_info():
        ...
        return render_template("rki/rki_info.html", page_info=page_info)
    ...
    def url_rki_test(page=1):
        ...
        try:
            ...
            page_data = None
        return render_template(
            "rki/test/rki_test.html", page_data=page_data, page_info=page_info
        )
    ...
    def url_rki_bundesland_one(bundesland_id: int, page: int = 1):
        ...
        try:
            ...
            page_info = WebPageContent(
                "RKI", "Bundesland", location_group.location_group
            )
        except OperationalError:
            flash("No date_reported in the database.")
            page_data = None
    ...
    def url_rki_landkreis_one(landkreis_id: int, page: int = 1):
        ...
        try:
            ...
            page_info = WebPageContent(
                "RKI", location.location_code + " " + location.location
            )
        except OperationalError:
            flash("No date_reported in the database.")
            page_data = None
...
class RkiTestUrls:
    ...
    def url_rki_test_rki_import_get_new_dates_reported_as_array():
        app.logger.info(
            " [RKI] url_rki_test_rki_import_get_new_dates_reported_as_array - START"
        )
        ...
        for date_reported in RkiImport.get_new_dates_reported_as_array():
            ...
            app.logger.info(line)
        app.logger.info(
            " [RKI] url_rki_test_rki_import_get_new_dates_reported_as_array - DONE"
        )
    ...
    def url_rki_test_rki_import_get_datum_of_all_import():
        app.logger.info(
            " [RKI] url_rki_test_rki_import_get_datum_of_all_import - START"
        )
    ...
    def url_rki_test_rki_service_service_update_rki_import_get_new_dates_as_array():
        app.logger.info(
            " [RKI] url_rki_test_rki_service_service_update_rki_import_get_new_dates_as_array - START"
        )
        flash(
            " [RKI] url_rki_test_rki_service_service_update_rki_import_get_new_dates_as_array - START"
        )
        for datum in rki_service.service_update.rki_import_get_new_dates_as_array():
            app.logger.info(str(datum))
        app.logger.info(
            " [RKI] url_rki_test_rki_service_service_update_rki_import_get_new_dates_as_array - DONE"
        )
        flash(
            " [RKI] url_rki_test_rki_service_service_update_rki_import_get_new_dates_as_array - DONE"
        )
    ...
    def url_rki_test_rki_test_service_full_update_dimension_tables():
        app.logger.info(
            " [RKI] url_rki_test_rki_test_service_full_update_dimension_tables - START: rki_test_service.full_update_dimension_tables()"
        )
        flash(
            " [RKI] url_rki_test_rki_test_service_full_update_dimension_tables - START: rki_test_service.full_update_dimension_tables()"
        )
        ...
        app.logger.info(
            " [RKI] url_rki_test_rki_test_service_full_update_dimension_tables - DONE: rki_test_service.full_update_dimension_tables()"
        )
        flash(
            " [RKI] url_rki_test_rki_test_service_full_update_dimension_tables - DONE: rki_test_service.full_update_dimension_tables()"
        )
    ...
    def url_rki_test_rki_test_service_full_update_get_altersgruppe_list():
        app.logger.info(
            " [RKI] url_rki_test_rki_test_service_full_update_get_altersgruppe_list - START: rki_test_service.full_update_dimension_tables()"
        )
        flash(
            " [RKI] url_rki_test_rki_test_service_full_update_get_altersgruppe_list - START: rki_test_service.full_update_dimension_tables()"
        )
        ...
        app.logger.info(
            " [RKI] url_rki_test_rki_test_service_full_update_get_altersgruppe_list - DONE: rki_test_service.full_update_dimension_tables()"
        )
        flash(
            " [RKI] url_rki_test_rki_test_service_full_update_get_altersgruppe_list - DONE: rki_test_service.full_update_dimension_tables()"
        )
